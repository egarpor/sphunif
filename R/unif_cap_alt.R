...
#' spherical cap of radius \eqn{r} around a direction \eqn{\boldsymbol{\mu}} on
#' \eqn{\mathbb{S}^{p-1}:=\{\mathbf{x}\in \mathbb{R}^p:||\mathbf{x}||=1\}},
#' \eqn{p > 1}. The density at \eqn{\mathbf{x} \in \mathbb{S}^{p-1}} is given by
#' \deqn{c_{p,r}\mathbb{I}_{[1-r, 1]}(\mathbf{x}'\boldsymbol{\mu})
#' \quad\text{with}\quad c_{p,r}:=\omega_{p-1}\left[1-F_{p}(1-r)\right]}
#' where \eqn{\boldsymbol{\mu}\in S^{p-1}} is the directional
#' mean, \eqn{r\in [0,\pi]} is the radius of the spherical cap about
#' \eqn{\boldsymbol{\mu}}, \eqn{\omega_{p-1}} is the surface area of
#' \eqn{\mathbb{S}^{p-1}}, and \eqn{F_p(x)} is the cdf of a
#' \eqn{B(\frac{1}{2},\frac{p-1}{2})} beta distribution.
#'
#' The angular function of the uniform cap distribution is
#' \eqn{g(t):=\mathbb{I}_{[1-r, 1]}(t)}. The associated projected density
#' is \eqn{\tilde{g}(v):=\omega_{p-2}c_{p,r}(1-v^2)^{\frac{p-3}{2}}
#' \mathbb{I}_{[1-r, 1]}(v)}.
#'
...
#' @param p dimension of the ambient space \eqn{R^p} that contains \eqn{S^{p-1}}.
#' A positive integer.
#' @param u vector of probabilities.
#' @param mu the directional mean \eqn{\boldsymbol{\mu}} of the distribution.
...
#' \item \code{r_unif_cap}: a matrix of size \code{c(n, p)} with the random sample.
...
#' \item \code{g_unif_cap}: a vector of size \code{length(t)} with the evaluated
#' angular function.
...
#' plot(r_noise * x, pch = 16, col = col[rank(d_unif_cap(x = x, mu = mu, r = r))],
