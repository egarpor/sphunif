  ...
  mle.F.sp <- function(data, df1) {
    if (df1 <= 0) {
      ...
      diff.log.pdf <- function(x, d_1, d_2) {
        ((d_1 * x + d_2) * digamma(d_1 / 2 + d_2 / 2) + (-d_1 * x - d_2) * log(d_1 * x + d_2) + (-d_1 * x - d_2) * digamma(d_2 / 2) + (d_1 * x + d_2) * log(d_2) + d_1 * (x - 1)) / (2 * d_1 * x + 2 * d_2)
      }
    ...
    if (type == "norm") {
      ...
      r2dist <- function(y, d, alpha0) {
        hilf <- function(x, y, d, alpha) {
          return(stabledist::pstable(y / x, alpha = alpha / 2, beta = 1, gamma = 2 * (cos(pi * alpha / 4))^(2 / alpha), delta = 0, pm = 1) * dchisq(x, df = d))
        }
