...
s.hyb.quan <- MonteCarlo(
  func = H0.quan, nrep = 10000, param_list = param_list,
  ncpus = 14
)
summary(s.hyb.quan)
time_END <- Sys.time()
...
for (n in c("samplesize=100", "samplesize=200")) {
  ...
  for (d in c("dimension=100", "dimension=200", "dimension=300")) {
    ...
    j.d <- j.d + 1
    q.s.hyb[j.n, j.d] <- sort(as.numeric(s.hyb.quan$results$RES[n, d, ]))[
      0.95 * 10000
    ]
...
time_START <- Sys.time()
s.hyb.quan <- MonteCarlo(
  func = H0.quan, nrep = 5000, param_list = param_list,
  ncpus = 14
)
summary(s.hyb.quan)
time_END <- Sys.time()
...
H1.s.hyb <- function(Choice = 1, samplesize = 100, dimension = 100) {
  ...
  stat_hyb <- function(X, Sobolev_vk2 = c(1, 0), u2 = 2 * (pi^2 / 3 - 3),
    ...
    mle.F.sp <- function(data, df1) {
      ...
      if (df1 <= 0) {
        ...
        diff.log.pdf <- function(x, d_1, d_2) {
          ...
          ((d_1 * x + d_2) * digamma(d_1 / 2 + d_2 / 2) + (-d_1 * x - d_2) *
            log(d_1 * x + d_2) + (-d_1 * x - d_2) * digamma(d_2 / 2) + (d_1 * x
            + d_2) * log(d_2) + d_1 * (x - 1)) / (2 * d_1 * x + 2 * d_2)
    ...
    for (j in 1:boot) {
      ...
      BS <- rf(n, df1 = p, df2 = para)
      BS_estimator <- mle.F.sp(BS, df1 = p)
      y[j] <- as.numeric(ad.test(
        BS,
        null = function(x) pf(x, df1 = p, df2 = BS_estimator)
      )$statistic)
  ...
  for (d in dimension) {
    ...
    for (n in samplesize) {
      ...
      sample5 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 1) *
          sqrt(d * matrix(rf(n, df1 = d, df2 = true.nu), nrow = n, ncol = d))
      sample6 <- rvmf(n,
        mu = c(1, rep(0, d - 1)),
        k = 3
      ) * sqrt(d * matrix(rf(n, df1 = d, df2 = true.nu),
        nrow = n, ncol = d
      ))
      sample7 <- rvmf(n,
        mu = c(1, rep(0, d - 1)),
      ...
      sample9 <- rmst(n,
        xi = mean_vec, Omega = diag(1, d), alpha = alpha1,
        nu = true.nu
      )
      sample10 <- rmst(n,
        xi = mean_vec, Omega = cov_matrix, alpha = alpha0,
        nu = true.nu
      )
...
s.hyb.alt.t.2 <- MonteCarlo(
  ...
  param_list = param_list, ncpus = 60
)
...
H0.quan <- function(samplesize = c(20, 50, 100), dimension = c(2, 3, 5)) {
  ...
  stat_hyb <- function(X, Sobolev_vk2 = c(1, 0), u2 = 2 * (pi^2 / 3 - 3),
    ...
    if (type == "norm") {
      ...
      r2dist <- function(y, d, alpha0) {
        ...
        hilf <- function(x, y, d, alpha) {

          return(stabledist::pstable(y / x,
            alpha = alpha / 2, beta = 1,
            gamma = 2 * (cos(pi * alpha / 4))^(2 / alpha), delta = 0,
            pm = 1
          ) * dchisq(x, df = d))
        ...
        for (i in 1:length(y)) {
          ...
          hilf.erg[i] <- integrate(hilf,
            y = y[i], d = d, alpha = alpha0,
            lower = 0, upper = Inf
          )$value
      ...
      radii_stat <- ad.test(x = radii_2, null = function(x) {
  ...
  for (d in dimension) {
    ...
    for (n in samplesize) {
      ...
      X <- mvrnorm(n, rep(0, d), diag(1, d))
      A <- stabledist::rstable(n, true.alpha0 / 2, 1,
        2 * (cos(pi * true.alpha0 / 4))^(2 /
          true.alpha0), 0,
        pm = 1
      )
      X <- matrix(sqrt(A), ncol = d, nrow = n) * X
      j.n <- j.n + 1
      Erg[j.d, j.n] <- stat_hyb(X,
        type = "stable",
        alpha0 = true.alpha0
      )$statistic
...
H1.s.hyb <- function(Choice = c(1, 2), samplesize = c(100, 200),
  ...
  stat_hyb <- function(X, Sobolev_vk2 = c(1, 0), u2 = 2 * (pi^2 / 3 - 3),
    ...
    return(list(
      ...
      p.value = 1 - pchisq(-2 * (log(projs_stat) + log(radii_stat)),
        df = 4
      ), proj = projs_stat, rad = radii_stat
    ))
  ...
  for (d in dimension) {
    ...
    for (n in samplesize) {
      ...
      X <- mvrnorm(n, rep(0, d), diag(1, d))
      A <- rstable(n, true.alpha0 / 2, 1,
        2 * (cos(pi * true.alpha0 / 4))^(2 / true.alpha0), 0,
        pm = 1
      )
...
H1.s.hyb <- function(Choice = 1, samplesize = 100, dimension = 100) {
  ...
  stat_hyb <- function(X, Sobolev_vk2 = c(1, 0), u2 = 2 * (pi^2 / 3 - 3),
    ...
    return(list(
      statistic = -2 * (log(projs_stat) + log(BS.p.value)),
      p.value = 1 - pchisq(-2 * (log(projs_stat) + log(BS.p.value)),
        df = 4
      )
    ))
  ...
  for (d in dimension) {
    ...
    for (n in samplesize) {
      ...
      sample0 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 0) * matrix(rgamma(
        n, 2,
        5
      ), nrow = n, ncol = d)
      sample1 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 0.25) * matrix(rchisq(n,
        df = 2
      ), nrow = n, ncol = d)
      sample2 <- rvmf(n,
        mu = c(1, rep(0, d - 1)),
        k = 0.25
      ) * matrix(abs(rcauchy(n, 2, 5)),
        nrow = n,
        ncol = d
      )
      sample3 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 0.5) * matrix(abs(rt(
        n,
        2
      )), nrow = n, ncol = d)
      sample4 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 10) * matrix(rchisq(n,
        df = 20
      ), nrow = n, ncol = d)
      sample5 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 5) * matrix(rchisq(n,
        df = d
      ), nrow = n, ncol = d)
      sample6 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 2) * matrix(rgamma(
        n, 2,
        5
      ), nrow = n, ncol = d)
      sample7 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 5) * matrix(rgamma(
        n, 2,
        5
      ), nrow = n, ncol = d)
      sample8 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 10) * matrix(rgamma(
        n, 2,
        5
      ), nrow = n, ncol = d)
      sample9 <- rvmf(n, mu = c(1, rep(0, d - 1)), k = 20) * matrix(rgamma(
        n, 2,
        5
      ), nrow = n, ncol = d)
...
xtable::xtable(t(t(round(Ergebnis, 2) * 100)),
  digits = 0,
